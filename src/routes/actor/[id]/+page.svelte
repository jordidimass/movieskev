    ...
    {#if data.gallery.length}
      <div class="mt-6 grid grid-flow-col gap-6 overflow-auto pb-2">
        {#each data.gallery as item}
          <article class="w-[220px]">
            <img class="h-[330px] w-full rounded-2xl object-cover" src={item.file_path ? `https://image.tmdb.org/t/p/original${item.file_path}` : "/img/guest.jpg"} alt="Portrait still" />
          </article>
        {/each}
      </div>
    {:else}
